...
func (r *Renderer) renderMonth(month time.Time, x, y int, selection *models.Selection) error {
	...
	for i, header := range dayHeaders {
		headerX := x + i*3 + 1
		r.terminal.Print(headerX, headerY, header, fg, bg)
	}
...
func (r *Renderer) getDayAttributes(date time.Time, selection *models.Selection) (fg, bg termbox.Attribute, text string) {
	...
	// Apply visual indicators based on state
	...
	if hasEvents {
		if dayNum < 10 {
			text = fmt.Sprintf(" %d*", dayNum) // Add asterisk for events
		} else {
			text = fmt.Sprintf("%d*", dayNum) // Add asterisk for events
		}
	}
...
func (r *Renderer) RenderEventList(date time.Time, events []models.Event) error {
	...
	r.terminal.PrintCentered(2, title, termbox.AttrBold, bg)

	// Draw separator
	separatorY := 4
	for i := 0; i < width; i++ {
		r.terminal.SetCell(i, separatorY, '-', fg, bg)
	}

	startY := 6
	if len(events) == 0 {
		r.terminal.PrintCentered(startY, "No events scheduled for this date", fg, bg)
	} else {
		for i, event := range events {
			if startY+i >= height-4 {
				// Too many events to display
				moreText := fmt.Sprintf("... and %d more events", len(events)-i)
				r.terminal.PrintCentered(startY+i, moreText, fg, bg)
				break
			}

			eventText := fmt.Sprintf("%s - %s", event.GetTimeString(), event.Description)

			// Truncate if too long
			if len(eventText) > width-4 {
				eventText = eventText[:width-7] + "..."
			}

			r.terminal.Print(2, startY+i, eventText, fg, bg)
		}
	}

	// Instructions
	instrY := height - 3
	r.terminal.PrintCentered(instrY, "A: add event  Esc: back to calendar", fg, bg)
